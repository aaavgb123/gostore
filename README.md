...
YNS is tightly integrated with [Stripe](https://stripe.com), so you need a Stripe account to use Go Store. Follow the instructions from Stripe to [create an account](https://dashboard.stripe.com/register).
...
After following the above steps, run `pnpm install` to install the required dependencies, and then run `pnpm dev` to start the development server on your machine. Go Store will be available at [localhost:3000](http://localhost:3000)

## Add products

Go Store gets all the products, prices, descriptions, and categories from Stripe. So, if you know Stripe already, you'll feel right at home!
...
Additionally, Go Store uses product metadata to provide more context information about the products. You can specify the following metadata fields:
...
Now you should see all added products in Go Store.

## Variants

Go Store supports simple product variants. To create a product with variants, you must add multiple products to Stripe with the same `slug` metadata field. YNS uses the `variant` metadata field to distinguish between different variants of the same product. For example, if you have a T-shirt in multiple sizes, you can create three products with the `slug` of `t-shirt` and `variant` values of `small`, `medium`, and `large`.
...
Go Store comes with a preview of Stripe Tax support. To enable it, set the `ENABLE_STRIPE_TAX` environment variable to any value (i.e., `1`).
...
console.log('所有商品slug:', products.map(p => p.metadata && p.metadata.slug));

const visibleProducts = products.filter(
  (product) => product.metadata.slug !== '哈哈哈'
);

console.log('最终显示的商品slug:', visibleProducts.map(p => p.metadata && p.metadata.slug));
